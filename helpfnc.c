...
//    char prefex_size[] = {48,48,48,48,48,48,48,49,13,10};
    ...
    if(rest_data_size <= 0xFF){
        sprintf(&hex[0],"%02x",rest_data_size);
        hex_size = 2;
    }else{
    ...
    memcpy(encoded_result + pos_index, hex,hex_size);
    pos_index += hex_size;
...
gzip_compress(void* input_data, size_t i_size, size_t* o_size)
{
    ...
    defstream.avail_out = (uInt)100000;
